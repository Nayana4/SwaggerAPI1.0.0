...
class TestDefaultController(BaseTestCase):
    ...
    def test_fecontent_get_sc_image(self):
        ...
        headers = [('Range', 'Range_example')]
        response = self.client.open(
            '/fenet2/1.0.0/fecontent/{scParameters}'.format(scParameters='scParameters_example'),
            method='GET',
            headers=headers)
        self.assert200(response,
                       'Response body is : ' + response.data.decode('utf-8'))
    ...
    def test_femeta_get_meta_data(self):
        ...
        headers = [('Range', 'Range_example')]
        response = self.client.open(
            '/fenet2/1.0.0/femeta/{metaParameters}'.format(metaParameters='metaParameters_example'),
            method='GET',
            headers=headers)
        self.assert200(response,
                       'Response body is : ' + response.data.decode('utf-8'))
